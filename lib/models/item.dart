...
class Item {
  ...
  set units(Map<String, dynamic>? newUnits) {
  ...
  void modifyLatestStockEntry(ItemTransaction transaction, double newNoOfItemsInTransaction,
  ...
  static List<Item> fromQuerySnapshot(QuerySnapshot snapshot) {
    ...
    for (var doc in snapshot.docs) {
      ...
      Item item = Item.fromMapObject(data);
      item.id = doc.id;
      items.add(item);
    }
    items.sort((a, b) {
  ...
  Item.fromMapObject(Map<String, dynamic> map) {
    ...
    this._totalStock = map['total_stock'] ?? 0.0;
    this._lastStockEntry = map['last_stock_entry'];
    this._used = map['used'] ?? 0;
