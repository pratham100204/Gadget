...
class _MonthlyHistoryState extends State<MonthlyHistory> {
  ...
  BottomAppBar _buildBottomAppBar(BuildContext context) {
    return BottomAppBar(
      ...
      child: Row(
        children: <Widget>[
          IconButton(
            icon: Icon(Icons.card_giftcard, color: Color(0xFFBDBDBD)),
            onPressed:
                () => WindowUtils.navigateToPage(
                  context,
                  target: 'Transactions',
                  caller: 'Month History',
                ),
          ),
          SizedBox(width: 20.0),
          IconButton(
            icon: Icon(Icons.access_alarm, color: Color(0xFFBDBDBD)),
            onPressed:
                () => WindowUtils.navigateToPage(
                  context,
                  target: 'Due Transactions',
                  caller: 'Month History',
                ),
          ),
          SizedBox(width: 150.0),
          IconButton(
            icon: Icon(Icons.history, color: Color(0xFFBDBDBD)),
            onPressed:
                () => WindowUtils.navigateToPage(
                  context,
  ...
  Widget showTransactionHistoryForCurrentMonth() {
    if (this.currentMonthHistory.isNotEmpty) {
          ...
          return Card(
            ...
            child: ListTile(
              leading: Column(
                ...
                children: <Widget>[SizedBox(height: 20.0), Text(date)],
              ),
  ...
  void _initializeCurrentMonthHistory() async {
    ...
    setState(() {
      allTransactions.forEach((ItemTransaction transaction) {
        DateTime transactionDate = DateFormat.yMMMd().add_jms().parse(
          transaction.date!,
        );
        ...
        if (transaction.type == 0 &&
          ...
          if (this.currentMonthHistory.containsKey(day)) {
